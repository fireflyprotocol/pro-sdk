...
/// struct for typed errors of method [`get_exchange_stats_all_time`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetExchangeStatsAllTimeError {
    Status500(models::Error),
    UnknownValue(serde_json::Value),
}
...
pub async fn get_exchange_stats_all_time(configuration: &configuration::Configuration, ) -> Result<models::StatsAllTimeResponse, Error<GetExchangeStatsAllTimeError>> {

    let uri_str = format!("{}/v1/exchange/stats/allTime", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::StatsAllTimeResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::StatsAllTimeResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetExchangeStatsAllTimeError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
