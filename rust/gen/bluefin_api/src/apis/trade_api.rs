...
pub async fn cancel_standby_orders(configuration: &configuration::Configuration, cancel_orders_request: models::CancelOrdersRequest) -> Result<models::CancelOrdersResponse, Error<CancelStandbyOrdersError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CancelOrdersResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CancelOrdersResponse`")))),
        }
    } else {
        let content = resp.text().await?;
