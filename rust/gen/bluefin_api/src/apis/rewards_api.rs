...
/// struct for typed errors of method [`get_user_bonuses`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetUserBonusesError {
    Status400(models::Error),
    UnknownValue(serde_json::Value),
}
...
/// struct for typed errors of method [`update_affiliate_ember_refferal_share`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum UpdateAffiliateEmberRefferalShareError {
    Status401(models::Error),
    Status404(models::Error),
    Status400(models::Error),
    Status500(models::Error),
...
pub async fn get_user_bonuses(configuration: &configuration::Configuration, epoch_number: i32) -> Result<Vec<models::KoraUserBonusResponse>, Error<GetUserBonusesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_epoch_number = epoch_number;

    let uri_str = format!("{}/v1/rewards/bonuses", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);

    req_builder = req_builder.query(&[("epochNumber", &p_epoch_number.to_string())]);
    if let Some(ref user_agent) = configuration.user_agent {
        req_builder = req_builder.header(reqwest::header::USER_AGENT, user_agent.clone());
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::KoraUserBonusResponse&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::KoraUserBonusResponse&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetUserBonusesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn update_affiliate_ember_refferal_share(configuration: &configuration::Configuration, update_affiliate_ember_refferal_share_request: models::UpdateAffiliateEmberRefferalShareRequest) -> Result<models::AffiliateMetadata, Error<UpdateAffiliateEmberRefferalShareError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_update_affiliate_ember_refferal_share_request = update_affiliate_ember_refferal_share_request;

    let uri_str = format!("{}/v1/rewards/affiliate/emberRefferalShare", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);
    ...
    req_builder = req_builder.json(&p_update_affiliate_ember_refferal_share_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UpdateAffiliateEmberRefferalShareError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
