...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetAccountValueHistoryByAccountError {
    Status400(models::Error),
    Status404(models::Error),
    Status500(models::Error),
    UnknownValue(serde_json::Value),
...
pub async fn get_account_value_history_by_account(configuration: &configuration::Configuration, account_address: &str) -> Result<models::AccountValueHistory, Error<GetAccountValueHistoryByAccountError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_account_address = account_address;

    let uri_str = format!("{}/api/v1/accounts/{accountAddress}/valueHistory", configuration.base_path, accountAddress=crate::apis::urlencode(p_account_address));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetAccountValueHistoryByAccountError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
